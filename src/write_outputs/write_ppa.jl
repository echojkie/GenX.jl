function write_vom_ppa(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    vom_ppa = value.(EP[:eVariableSubsidiesByPlant])
    vom_ppa *= scale_factor

    df_vom = DataFrame(Resource = resources,
        Zone = zones,
        AnnualSum = zeros(G))
    df_vom.AnnualSum .= vom_ppa * weight

    write_temporal_data(df_vom, vom_ppa, path, setup, "vom_ppa")
    return df_vom
end

function write_fom_ppa(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    gen = inputs["RESOURCES"]   # Resources (objects)
    resources = inputs["RESOURCE_NAMES"]    # Resource names
    zones = zone_id.(gen)
    ...
    fom_ppa = value.(EP[:eFixedSubsidiesByPlant])
    fom_ppa *= scale_factor

    df_fom = DataFrame(Resource = resources,
        Zone = zones,
        AnnualSum = zeros(G))
    df_fom.AnnualSum .= fom_ppa * weight

    write_temporal_data(df_fom, fom_ppa, path, setup, "fom_ppa")
    return df_fom
end

function write_real_cost(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ## Cost results
    ...
    ELECTROLYZER_ALL = !isempty(VS_ELEC) ? union(VS_ELEC, inputs["ELECTROLYZER"]) :
                       inputs["ELECTROLYZER"]
    @expression(EP, eCost_real, 
                EP[:eObj] + EP[:eTotalVariableSubsidies] + EP[:eTotalFixedSubsidies])
    ...
    CSV.write(joinpath(path, "costs_ppa.csv"), dfCost)
end
